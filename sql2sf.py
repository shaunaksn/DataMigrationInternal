...
print("input: " + str(args.inputfile.name), file=sys.stderr)
print("output: " + str(args.outputfile.name), file=sys.stderr)

json_file_path = os.path.abspath("/Users/DELL/Desktop/Sample User Data.json")
database_type = 'MySQL'
file_type = 'json'
file_name = 'Sample User Data.json'
table_name = 'json_ddl'

with open(json_file_path, mode='rb') as file_content:
    converter = sqlizer.File(file_content, database_type, file_type, file_name, table_name)
    converter.convert(wait=True)
    output_sql = converter.download_result_file().text.split('INSERT')[0]
    print(output_sql)


output= make_snow(output_sql, args.outputfile, args.no_comments)
print(output)
...
print("done translating " + args.inputfile.name, file=sys.stderr)
